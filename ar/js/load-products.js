 ...
 * Dynamically loads, filters, and renders product catalog for ar.
 */
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const contactPath = `${origin}/ar/contact/`;
    const tcoPath = `${origin}/ar/tco-calculator/`;
    ...
    fetch(jsonPath, { cache: 'no-store' })
        ...
        .catch(() => {
            productContainer.innerHTML = `<div class="col-12 text-center text-danger">Error loading product catalog. Please contact us directly. <a href="${contactPath}" class="text-danger text-decoration-underline">طلب عينات ماسترباتش مجانية واختبار مطابقة الألوان</a></div>`;
        });
});
                ...
                <div class="card h-100 border-0 shadow-sm product-catalog-card">
                    <div class="card-body p-4">
                        ...
                        <a href="${contactPath}?product=${encodeURIComponent(product.product_code)}" class="btn btn-outline-primary w-100 rounded-pill py-2 fw-bold mb-3">
                            طلب عينات ماسترباتش مجانية واختبار مطابقة الألوان
                        </a>
                        <div class="text-center">
                            <a href="${tcoPath}" class="text-decoration-none small text-muted tco-hint-link">
                                <i class="fas fa-calculator me-1"></i>طلب عينات ماسترباتش مجانية واختبار مطابقة الألوان
                            </a>
                        </div>
                    </div>
                </div>
