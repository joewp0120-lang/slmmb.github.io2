 ...
 * Dynamically loads, filters, and renders product catalog for id.
 */
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const contactPath = `${origin}/id/contact/`;
    const tcoPath = `${origin}/id/tco-calculator/`;
    ...
    fetch(jsonPath, { cache: 'no-store' })
        ...
        .catch(() => {
            productContainer.innerHTML = `<div class="col-12 text-center text-danger">Error loading product catalog. Please contact us directly. <a href="${contactPath}" class="text-danger text-decoration-underline">Minta Sampel Masterbatch Gratis & Uji Pencocokan Warna</a></div>`;
        });
});
                ...
                <div class="card h-100 border-0 shadow-sm product-catalog-card">
                    <div class="card-body p-4">
                        ...
                        <a href="${contactPath}?product=${encodeURIComponent(product.product_code)}" class="btn btn-outline-primary w-100 rounded-pill py-2 fw-bold mb-3">
                            Minta Sampel Masterbatch Gratis & Uji Pencocokan Warna
                        </a>
                        <div class="text-center">
                            <a href="${tcoPath}" class="text-decoration-none small text-muted tco-hint-link">
                                <i class="fas fa-calculator me-1"></i>Minta Sampel Masterbatch Gratis & Uji Pencocokan Warna
                            </a>
                        </div>
                    </div>
                </div>
