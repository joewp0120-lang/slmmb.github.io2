/**
 * Localized load-products.js
 * Dynamically loads, filters, and renders product catalog for es.
 */
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const contactPath = `${origin}/es/contact/`;
    const tcoPath = `${origin}/es/tco-calculator/`;
    ...
    fetch(jsonPath, { cache: 'no-store' })
        ...
        .then(products => {
            ...
            if (categoryFilter) categoryFilter.addEventListener('change', () => filterAndRender(productContainer, contactPath, tcoPath));
            if (materialFilter) materialFilter.addEventListener('change', () => filterAndRender(productContainer, contactPath, tcoPath));
            if (resetBtn) {
        ...
        .catch(() => {
            productContainer.innerHTML = `<div class="col-12 text-center text-danger">Error loading product catalog. Please contact us directly. <a href="${contactPath}" class="text-danger text-decoration-underline">Solicitar Muestras de Masterbatch Gratis y Prueba de Igualación de Color</a></div>`;
        });
});
...
function filterAndRender(container, contactPath, tcoPath) {
    ...
    const filtered = allProducts.filter(product => {
        ...
        const matchMaterial = materialVal === 'all' || (product.materials && product.materials.toUpperCase().includes(materialVal.toUpperCase()));
                ...
                <div class="card h-100 border-0 shadow-sm product-catalog-card">
                    <div class="card-body p-4">
                        ...
                        <a href="${contactPath}?product=${encodeURIComponent(product.product_code)}" class="btn btn-outline-primary w-100 rounded-pill py-2 fw-bold mb-3">
                            Solicitar Muestras de Masterbatch Gratis y Prueba de Igualación de Color
                        </a>
                        <div class="text-center">
                            <a href="${tcoPath}" class="text-decoration-none small text-muted tco-hint-link">
                                <i class="fas fa-calculator me-1"></i>Solicitar Muestras de Masterbatch Gratis y Prueba de Igualación de Color
                            </a>
                        </div>
                    </div>
                </div>
