document.addEventListener('DOMContentLoaded', function() {
    ...
    if (navCollapse) {
        var syncNavMenuState = function() {
            document.body.classList.toggle('nav-menu-open', navCollapse.classList.contains('show'));
        };

        navCollapse.addEventListener('show.bs.collapse', syncNavMenuState);
        navCollapse.addEventListener('shown.bs.collapse', syncNavMenuState);
        navCollapse.addEventListener('hide.bs.collapse', syncNavMenuState);
        navCollapse.addEventListener('hidden.bs.collapse', syncNavMenuState);

        var navObserver = new MutationObserver(syncNavMenuState);
        navObserver.observe(navCollapse, { attributes: true, attributeFilter: ['class'] });
        syncNavMenuState();
    }
    ...
    (function() {
        ...
        whatsappLink.href = 'https://wa.me/8618027593021?text=Hola%20SLMMB%2C%20necesito%20muestras%20gratis%20de%20masterbatch%20y%20soporte%20para%20igualaci%C3%B3n%20de%20color%20para%20mi%20proyecto%20pl%C3%A1stico.';
        whatsappLink.className = 'whatsapp-float';
        whatsappLink.target = '_blank';
        whatsappLink.rel = 'noopener';
        whatsappLink.setAttribute('aria-label', 'Chatear por WhatsApp');
        whatsappLink.innerHTML = '<i class="fab fa-whatsapp whatsapp-icon"></i>';

        var sampleLink = document.createElement('a');
        sampleLink.href = '/es/contact/?intent=sample';
        sampleLink.className = 'sample-float';
        sampleLink.setAttribute('aria-label', 'Solicitar Muestras de Masterbatch Gratis y Prueba de Igualación de Color');
        sampleLink.innerHTML = '<i class="fas fa-gift me-2"></i><span class="sample-float-text">Solicitar Muestras de Masterbatch Gratis y Prueba de Igualación de Color</span>';
